...
import { ArrowLeft, ArrowRight, Check, Cog, Container, Cloud, Database, Smartphone, Bell, ChevronDown, GitPullRequest, Rocket } from "lucide-react";
...
const CaseStudy = () => {
  ...
  const go = (i: number) => {
    ...
    if (!el) return;
    const target = Math.max(0, Math.min(5, i));
    ...
    <div className="fixed inset-0 bg-[#0f172a] text-slate-100 font-sans">
      ...
      <div className="md:hidden fixed top-16 left-0 right-0 z-40 h-1 bg-white/5">
        <div
          ...
          style={{ width: `${((index + 1) / 6) * 100}%`, background: TEAL }}
        />
      </div>
      ...
      {/* Slides container — native snap scroll (works on web + mobile) */}
      <div
        ref={scrollerRef}
        className="h-full overflow-y-auto snap-y snap-mandatory scroll-smooth"
      >
        <section className="w-full h-screen snap-start relative overflow-hidden">
          <HeroSlide study={study} onScroll={() => go(1)} />
        </section>
        {sections.map((s, i) => (
          <section key={s.id} className="w-full h-screen snap-start relative overflow-hidden">
  ...
  return (
    <div className="w-full h-full bg-[#0f172a] relative overflow-hidden">
      ...
      <div className="relative h-full container mx-auto px-6 md:px-12 flex flex-col md:flex-row items-center gap-12 pt-24 md:pt-0">
        ...
        <div className="hidden md:block flex-1 md:max-w-[40%] w-full">
          <PipelineHero />
        </div>
      </div>
      <button
...
const SectionSlide = ({
  ...
  return (
    <div className="relative w-full h-full overflow-y-auto md:overflow-hidden" style={{ background: bg }}>
      ...
      <Watermark n={n} />
      <div className="relative min-h-full container mx-auto px-6 md:px-12 flex items-center py-24 md:py-0">
    ...
    <div className="grid md:grid-cols-2 gap-12 items-center w-full">
      ...
      <div className="space-y-3">
        {section.items?.map((item, i) => (
          <div
            key={item}
            className="flex items-center gap-4 p-4 rounded-xl bg-[#1e293b] border-l-4 border-red-500/70 animate-fade-up"
...
// Solution: high-level flow — what a PR triggers, end-to-end
  ...
  <div className="w-full space-y-10">
    ...
    <div className="hidden md:flex items-stretch gap-3">
      {PIPELINE_STEPS.map((s, i) => (
        <div key={s.label} className="flex items-center gap-3 flex-1">
          <div
            className="flex-1 p-5 rounded-xl bg-[#1e293b] border text-center animate-fade-up"
            style={{ borderColor: `${TEAL}55`, boxShadow: `0 0 24px ${TEAL}15`, animationDelay: `${i * 150}ms` }}
          >
            <div
              className="h-10 w-10 mx-auto rounded-full flex items-center justify-center mb-3"
          ...
          {i < PIPELINE_STEPS.length - 1 && (
            <ArrowRight className="h-5 w-5 shrink-0" style={{ color: TEAL }} />
          )}
        </div>
      ))}
    </div>
    ...
    <div className="md:hidden space-y-3">
      {PIPELINE_STEPS.map((s, i) => (
        <div
          key={s.label}
          className="flex items-center gap-4 p-4 rounded-xl bg-[#1e293b] border animate-fade-up"
          style={{ borderColor: `${TEAL}55`, animationDelay: `${i * 100}ms` }}
        >
          <div className="h-10 w-10 shrink-0 rounded-full flex items-center justify-center" style={{ background: `${TEAL}22`, color: TEAL }}>
            <s.icon className="h-5 w-5" />
          </div>
          <div>
            <div className="text-white font-semibold text-sm">{s.label}</div>
...
// Implementation: the actual tools & tech behind the flow
const IMPL_CARDS = [
  ...
  <div className="w-full space-y-10">
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      {IMPL_CARDS.map((c, i) => (
        <div
          key={c.title}
          className="p-5 rounded-xl bg-[#1e293b] border border-[#334155] hover:border-[#475569] transition-colors animate-fade-up"
